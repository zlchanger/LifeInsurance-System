...
/**
 * Servlet implementation class LIFEA004Servlet
 */
@WebServlet("/LIFEA004Servlet")
public class LIFEA004Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final String errJsp = "/LIFEA/LIFEA004.jsp";
	...
	private String task(HttpServletRequest request,HttpServletResponse response){
		String choose = request.getParameter("keknoAlter");
		String uktkno = request.getParameter("uktkno");
		String kekno = request.getParameter("kekno");
		StringBuffer sb = new StringBuffer(); 
		// ÐÞÕý
		if (DConstants.keknoAlter.equals(choose)) {
			if ((uktkno != null && uktkno != "") && (kekno == null || kekno == "")) {
			...
			} else if (kekno != null && kekno != "") {
				sb.append("select * from DHISTORY where KEKNO = '").append(kekno).append("' and DEFEFLG = '0'");
				if (uktkno != null && uktkno != "") {
					sb.append(" and UKTKNO = '").append(uktkno).append("'");
				}
				Vector<Hashtable<String, String>> datas = new Vector<Hashtable<String, String>>();
				try {
					datas = db.getVector(sb.toString());
				...
				request.setAttribute("nextform", form);
				request.setAttribute("domessage", DConstants.keknoAlter);
				nextJsp = "/LIFEA/LIFEA005.jsp";
				return nextJsp;
			}
		} else if (DConstants.keknoAgian.equals(choose)) {
			if ((uktkno != null && uktkno != "") && (kekno == null || kekno == "")) {
				sb.append("select * from DHISTORY where UKTKNO = '").append(uktkno).append("' and DEFEFLG = '0'");
				Vector<Hashtable<String, String>> datas = new Vector<Hashtable<String, String>>();
				try {
				...
				form.setUkdata(datas);
				request.setAttribute("nextform", form);
				request.setAttribute("domessage", DConstants.keknoAgian);
				nextJsp = "/LIFEA/LIFEA005.jsp";
				return nextJsp;
			}
		} else if (DConstants.keknoDele.equals(choose)) {
			if ((uktkno != null && uktkno != "") && (kekno == null || kekno == "")) {
				sb.append("select * from DHISTORY where UKTKNO = '").append(uktkno).append("' and DEFEFLG = '0'");
				Vector<Hashtable<String, String>> datas = new Vector<Hashtable<String, String>>();
				try {
				...
				form.setUkdata(datas);
				request.setAttribute("nextform", form);
				request.setAttribute("domessage", DConstants.keknoDele);
				nextJsp = "/LIFEA/LIFEA005.jsp";
				return nextJsp;
			} else if (kekno != null && kekno != "") {
				sb.append("select * from DHISTORY where KEKNO = '").append(kekno).append("' and DEFEFLG = '0'");
