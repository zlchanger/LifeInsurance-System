...
<script type="text/javascript" language="javascript"
	src="LIFEC/LIFEC003.js" charset="utf-8"></script>
<title>契约者情报返送录入(LIFEC003)</title>
</head>
<%
...
<body>
<form name="form1">
		<input type="hidden" name="sid" value="LIFEC003">
		<div>&nbsp;</div>
		...
		<tr><td>&nbsp;</td></tr>
			<tr><td width="5%">&nbsp;</td>
			<td width="12%">契约者姓：</td>
			<td width="30%"><input type="text" name="kamanax" <%=data.disableName() %> value=<%=(String)data.getKamanax() %>></td>
			<td width="5%">&nbsp;</td>
			<td width="12%">契约者名：</td>
			<td width="36%"><input type="text" name="kamanam" <%=data.disableName() %> value=<%=(String)data.getKamanam() %>></td></tr>
			<tr><td>&nbsp;</td>
			<td>生年月日：</td>
			<td><input type="text" name="birthday" <%=data.disableBir() %> value=<%=(String)data.getBirthday() %>></td>
			<td>&nbsp;</td>
			<td>契约者性别：</td>
			<td><input type="text" name="sex" <%=data.disableSex() %> value=<%=(String)data.getSex() %>></td></tr>
			<tr><td width="5%">&nbsp;</td>
			<td width="12%">契约者电话：</td>
			<td width="30%"><input type="text" name="tele" maxlength="13" <%=data.disableTele() %> value=<%=(String)data.getTele() %>></td>
			<td width="5%">&nbsp;</td>
			<td width="12%">&nbsp;</td>
			<td width="36%">&nbsp;</td></tr>
			<tr><td>&nbsp;</td>
			<td>契约者邮编：</td>
			<td><input type="text" name="zipcode" maxlength="6" <%=data.disableWhe() %> value=<%=(String)data.getZipcode() %>></td>
			<td>&nbsp;</td>
			<td>契约者住址：</td>
			<td><input type="text" name="addre" <%=data.disableWhe() %> value=<%=(String)data.getAddre() %>></td>
			</tr>
			<tr><td>&nbsp;</td>
			<td>契约者身体状况：</td>
			<td><input type="text" name="physicalcase" <%=data.disableOth() %> value=<%=(String)data.getPhysicalcase() %>></td>
			<td colspan="3">&nbsp;</td></tr>
			<tr><td>&nbsp;</td>
			<td>契约者职业：</td>
			<td><input type="text" name="occupation" <%=data.disableOth() %> value=<%=(String)data.getOccupation() %>></td>
			<td colspan="3">&nbsp;</td></tr>
			<tr><td>&nbsp;</td></tr>
		</table></td></tr>
		</table>
