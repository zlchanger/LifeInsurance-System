...
<script src="LIFEE/LIFEE001.js" type="text/javascript"></script>
<title>履历修正(LIFEE001)</title>
</head>
<%
	LIFEE001Form form = (LIFEE001Form) request.getAttribute("nextform");
	String kanama = form.getKanama();
	kanama = kanama.substring(0,1) + kanama.substring(2);
%>
<body>
	<form name="form1">
		<input type="hidden" name="sid" value="LIFEE001">
		...
		<tr><td>&nbsp;</td>
		<td>*契约者：</td>
		<td><input type="text" name="kanama" id="kanama" value=<%=kanama %>></td>
		<td>&nbsp;</td>
		<td>*受付内容：</td>
		<td><input type="text" name="ukContent" id="ukContent" size="30" value=<%=form.getUkContent() %>></td>
		<td>&nbsp;</td></tr>
		<tr><td>&nbsp;</td></tr>
		<tr><td colspan="7">------担当者-----------------------------------------------------</td></tr>
		<tr><td>&nbsp;</td>
		<td>事务所CD:</td>
		<td><input type="text" name="jjscd" id="jjscd" value=<%=form.getJjscd() %>></td>
		<td>&nbsp;</td></tr>
		<tr><td>&nbsp;</td>
		<td>用户名：</td>
		<td><input type="text" name="userid" id="userid" value=<%=form.getUserid()%>></td></tr>
		<tr><td>&nbsp;</td></tr>
		</table>
		</td></tr>
		</table>
		<table width="100%" border="0" align="center">
			...
			<td width="10%">&nbsp;</td>
				<td width="20%"><input
					type="button" name="back" id="back" onclick="onBack(form1)"
